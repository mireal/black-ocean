...
import TestimonialBlock, { TestimonialContent } from "../entities/TestimonialBlock"
...
function TestimonialsContent({ content }: { content: TestimonialContent[] }) {
    ...
    const testimonialBlocks = content.slice(0,limit).map((item, index) => (
        <TestimonialBlock content={item} key={index} />
    ))
    ...
    return (
        <>
            ...
            <StyledButtonDark endIcon={buttonIcon} onClick={handleClick} sx={{width:'100%'}}>
                {buttonLabel}
            </StyledButtonDark>
        </>
    )
